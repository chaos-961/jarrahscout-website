...
hardcodes a hex. Fonts are Fraunces (display), Inter (body) and Noto Kufi
Arabic, through `next/font`. The first two are latin only, so every Arabic
string carries `font-arabic` and `lang="ar"`; a `dir="rtl"` block also needs
`w-fit`, or it right-aligns itself across whatever column it sits in.

`public/og.jpg` is the link preview, 1200x630. It is a rendered page rather
than a hand-drawn asset: the source markup is in the commit that added it, and
it is regenerated by screenshotting that at 1200x630.
...
- `maplibre-gl.css` is imported by the map components, which are dynamic, so
  its sheet lands after `globals.css` and wins any specificity tie. The map
  overrides in `globals.css` are deliberately doubled up for that reason.
- The header avoids framer-motion on purpose. It is in the root layout, so
  anything it imports is on every page's critical path.
